 ...
 * Only works for non-negative numbers (>=0)
 *
 */
public class AnyBaseToAnyBase {
	...
	public String base2base(String n, int b1, int b2) {
		if (b1 > MAXIMUM_BASE || b1 < MINIMUM_BASE ||
			b2 > MAXIMUM_BASE || b2 < MINIMUM_BASE ) {
			
			return("Invalid base!");
		}
		if (!validForBase(n, b1)) {
			return("The number is invalid for this base!");
		}
		
		// Declare variables: decimal value of n, 
		// character of base b1, character of base b2,
		// and the string that will be returned.		
		long decimalValue = 0, charB2;
		char charB1;
		String output="";
		// Immediate return of zero value, as it behaves 
		// unpredictably if entered to the conversion loops
		...
		while (decimalValue != 0) {
			...
			if (decimalValue % b2 < 10) 
				output = Long.toString(decimalValue % b2) + output;
